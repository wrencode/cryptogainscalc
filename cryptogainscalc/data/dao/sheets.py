...
logger = get_logger(__file__, log_dir=Path(__file__).parent.parent.parent.parent / "logs")
...
class SheetsTable(BaseTable):
    def __init__(
        ...
        sheet_id: str,
        tab_name: str,
        starting_range_col: str,
        ending_range_col: str,
        starting_range_row: str = "",
        ending_range_row: str = "",
    ):
        ...
        google_auth_dir = Path(__file__).parent.parent.parent.parent / "auth"
        ...
        try:
            ...
            if not values:
                ...
                taxable_tx_types = set(
                    raw_df[raw_df["Taxable Event Tx #"] != "--"]["Tx Type"].tolist()
                )
                # logger.info(f"\n{raw_df.head(2).to_string()}")
                ...
                self.data["currency_in_fiat_price"] = self.data[
                    "currency_in_fiat_price"
                ].apply(lambda x: abs(float(Decimal(x))))
                self.data["currency_out"] = self.data["currency_out"].str.upper()
                self.data["currency_out_fiat_price"] = self.data[
                    "currency_out_fiat_price"
                ].apply(lambda x: abs(float(Decimal(x))))
                self.data["tx_taxable"] = self.data["tx_taxable"] != "--"
                self.data["currency_in_volume"] = (
                ...
                #     "currency_in_volume"
                # ] = self.data["fiat_value"] / self.data["currency_in_fiat_price"]
                self.data_col_index_map["currency_in_volume"] = (
                    max(self.data_col_index_map.values()) + 1
                )

                self.data["currency_out_volume"] = (
                ...
                #     "currency_out_volume"
                # ] = self.data["fiat_value"] / self.data["currency_out_fiat_price"]
                self.data_col_index_map["currency_out_volume"] = (
                    max(self.data_col_index_map.values()) + 1
                )
                ...
                self.data_col_index_map["description"] = (
                    max(self.data_col_index_map.values()) + 1
                )
