...
from cryptogainscalc.models.attributes import TxType
from cryptogainscalc.models.transactions import Buy, Sell, Trade, Transact
...
class BaseTable(ABC):
    ...
    def _create_tx_class_instance_list(
        self, class_type: Type, df: DataFrame
    ) -> List[Union[Buy, Sell, Trade, Transact]]:
        txs = []
        for row in df.itertuples():
            txs.append(
                class_type(
                    ...
                    fiat_value=round(
                        float(row[self.data_col_index_map["fiat_value"]]), RD
                    ),
                    fiat_tx_fee=round(
                        float(row[self.data_col_index_map["fiat_tx_fee"]]), RD
                    ),
                    currency_in=row[self.data_col_index_map["currency_in"]],
                    currency_in_fiat_price=round(
                        float(row[self.data_col_index_map["currency_in_fiat_price"]]),
                        RD,
                    ),
                    currency_in_volume=round(
                        float(row[self.data_col_index_map["currency_in_volume"]]), RD
                    ),
                    currency_out=row[self.data_col_index_map["currency_out"]],
                    currency_out_fiat_price=round(
                        float(row[self.data_col_index_map["currency_out_fiat_price"]]),
                        RD,
                    ),
                    currency_out_volume=round(
                        float(row[self.data_col_index_map["currency_out_volume"]]), RD
                    ),
            ...
            if ticker != self.fiat_currency.lower():
                self.cryptocurrencies.add(
                    str.lower(row[self.data_col_index_map["currency_out"]])
                )
            self._tx_count += 1
        return txs
    ...
    def _get_transactions(
        ...
        if tax_year:
            df = self.data[
                ...
                & (
                    to_datetime(self.data["tx_timestamp"], format="%m/%d/%Y")
                    < f"{tax_year + 1}-01-01 00:00:00"
                )
            ]
        else:
            ...
            sorted(
                list(
                    chain(
                        ...
                        self.get_buys_from_trades(
                            tax_year, fiat_currency, sort_field, sort_direction
                        ),
                    )
                ),
                key=lambda x: getattr(x, sort_field),
    ...
    def get_sell_events(
        self, tax_year: int, fiat_currency: str, sort_field: str, sort_direction: str
    ) -> List[Union[Sell, Transact]]:
        return (
            sorted(
                list(
                    chain(
                        self.get_sells(tax_year, sort_field, sort_direction),
                        self.get_sells_from_trades(
                            tax_year, fiat_currency, sort_field, sort_direction
                        ),
                    )
                ),
