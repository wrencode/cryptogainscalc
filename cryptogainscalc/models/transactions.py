...
class Transaction(PythonObjectJson):
    def __init__(
        self,
        tx_id: int,
        tx_type: TxType,
        ...
        fiat_tx_fee: float,
        currency_in: str,
        currency_in_volume: float,
        currency_in_fiat_price: float,
        currency_out: str,
        currency_out_volume: float,
        currency_out_fiat_price: float,
        taxable: bool,
        ...
        if isinstance(timestamp, str):
            self.timestamp: datetime = datetime.strptime(timestamp, "%m/%d/%Y")
        else:
...
class Buy(Transaction):
    def __init__(
        ...
        super().__init__(
            ...
            currency_in,
            currency_in_volume,
            currency_in_fiat_price,
            currency_out,
            currency_out_volume,
            currency_out_fiat_price,
            taxable,
            description,
        )
...
class TaxableTransaction(object):
    def __init__(
        self, cryptocurrency: str, tx_in: Transaction, tx_out: Union[Transaction, None]
    ):
        self.cryptocurrency = cryptocurrency
        self.tx_in = tx_in
        self.tx_out = tx_out
        self.date_acquired = self.tx_in.timestamp if self.tx_in else None
        self.date_acquired_str = (
            self.tx_in.timestamp.strftime("%m/%d/%Y") if self.tx_in else None
        )
        self.date_sold = self.tx_out.timestamp if self.tx_out else None
        self.date_sold_str = (
            self.tx_out.timestamp.strftime("%m/%d/%Y") if self.tx_out else None
        )
        self.sales_proceeds = (
            round(self.tx_out.fiat_value - self.tx_out.fiat_tx_fee)
            if self.tx_out
            else None
        )
        self.cost_basis = (
            round(self.tx_in.fiat_value - self.tx_in.fiat_tx_fee)
            if self.tx_in
            else None
        )
        self.short_term = (
        ...
        self.long_term = not self.short_term if self.short_term is not None else None
        self.capital_gain_or_loss = (
            round(self.sales_proceeds - self.cost_basis)
            if self.sales_proceeds and self.cost_basis
            else None
        )
