...
from cryptogainscalc.data.extract import TxData
from cryptogainscalc.models.transactions import Buy, Sell, TaxableTransaction, Transact, Transaction

logger = get_logger(__file__, log_dir=Path(__file__).parent.parent.parent / "logs")
...
def split_unequal_tx(
    smaller_volume_tx: Transaction,
    greater_volume_tx: Transaction,
    greater_volume_tx_flow: TxFlow,
):
    opposite_flow: Optional[TxFlow] = None
    for flow in TxFlow:
        if flow.value != greater_volume_tx_flow.value:
            opposite_flow = flow

    greater_volume_tx_processed_pct = getattr(
        smaller_volume_tx, f"currency_{greater_volume_tx_flow.value}_volume"
    ...
    greater_volume_tx_processed_split.currency_in_volume = round(
        float(
    ...
    greater_volume_tx_processed_split.currency_out_volume = round(
        float(
    ...
    greater_volume_tx_processed_split.fiat_value = round(
        float(
    ...
    greater_volume_tx_processed_split.fiat_tx_fee = round(
        float(
    ...
    greater_volume_tx_unprocessed_split.currency_in_volume = round(
        float(
            greater_volume_tx_unprocessed_split.currency_in_volume
            * greater_volume_tx_unprocessed_pct
        ),
        RD,
    )
    greater_volume_tx_unprocessed_split.currency_out_volume = round(
        float(
    ...
    greater_volume_tx_unprocessed_split.fiat_value = round(
        float(
    ...
    greater_volume_tx_unprocessed_split.fiat_tx_fee = round(
        float(
...
class TaxableCrypto(object):
    def __init__(
        ...
        expenditure_types: List[str] = None,
    ):
        self.tax_year = tax_year
        self.fiat_currency = fiat_currency
        self.sort_field = sort_field
        self.sort_direction = sort_direction
        self.expenditure_types = (
            expenditure_types if expenditure_types else ["purchase", "donation", "gift"]
        )
        self.capital_gains_and_losses = []
        self.taxable_income = defaultdict(list)

    @staticmethod
    def _get_df_from_tx_list(
        taxable_txs: List[TaxableTransaction], exclude_columns: List[str] = None
    ):
        taxable_txs_df = concat(
            [taxable_tx.to_pd_series() for taxable_tx in taxable_txs], axis=1
        ).T
    ...
    def get_capital_gains_and_losses(self):
        ...
        buys = tx_data.retrieve_buy_events(
            self.tax_year, self.fiat_currency, self.sort_field, self.sort_direction
        )
        sells = tx_data.retrieve_sell_events(
            self.tax_year, self.fiat_currency, self.sort_field, self.sort_direction
        )
        transacts = tx_data.retrieve_transact_events(
            self.tax_year, self.sort_field, self.sort_direction
        )

        crypto_tallies = {
            ticker: {"in": [], "out": []}
            for ticker in tx_data.get_cryptocurrencies()
            if ticker.lower() not in self.expenditure_types
        }
        ...
        for ticker, tallies in crypto_tallies.items():
            ...
            txs_in: List[Transaction] = tallies["in"]
            txs_out: List[Transaction] = tallies["out"]

            logger.info(
                f"Transactions for {ticker}: {len(txs_in)} IN, {len(txs_out)} OUT"
            )

            if len(txs_in) == 0 or len(txs_out) == 0:
                continue
            ...
            while len(txs_out) > 0:
                ...
                if txs_in:
                    ...
                    # when "txns in" are missing sufficient volume to account for "txns out" then assume dividends
                    first_tx_in = Transact(
                        0,
                        first_tx_out.timestamp,
                        first_tx_out.fiat_value,
                        0,
                        "DIVIDEND",
                        first_tx_out.fiat_value,
                        1,
                        first_tx_out.currency_in,
                        first_tx_out.currency_in_volume,
                        first_tx_out.currency_in_fiat_price,
                        True,
                        "UNDOCUMENTED",
                    )
                ...
                if first_tx_out.timestamp < first_tx_in.timestamp:
                    raise IndexError(
                        "Currency out transaction detected before currency in transaction, check data!"
                    )
                ...
                if first_tx_in.currency_out_volume == first_tx_out.currency_in_volume:
                    ...
                    running_total_ticker_volume += (
                        first_tx_in.currency_out_volume
                        - first_tx_out.currency_in_volume
                    )
                    ...
                    if self.tax_year:
                        if first_tx_out.timestamp.year == self.tax_year:
                            self.capital_gains_and_losses.append(
                                TaxableTransaction(ticker, first_tx_in, first_tx_out)
                            )
                    else:
                        self.capital_gains_and_losses.append(
                            TaxableTransaction(ticker, first_tx_in, first_tx_out)
                    ...
                    first_tx_in_sold_split, first_tx_in_unsold_split = split_unequal_tx(
                        smaller_volume_tx=first_tx_out,
                        greater_volume_tx=first_tx_in,
                        greater_volume_tx_flow=TxFlow.IN,
                    )

                    split_txs_in.append(first_tx_in_sold_split)
                    split_txs_out.append(first_tx_out)

                    running_total_tx_in_volume += (
                        first_tx_in_sold_split.currency_out_volume
                    )
                    running_total_tx_out_volume += first_tx_out.currency_in_volume
                    running_total_ticker_volume += (
                        first_tx_in_sold_split.currency_out_volume
                    ...
                    if self.tax_year:
                        if first_tx_out.timestamp.year == self.tax_year:
                            self.capital_gains_and_losses.append(
                                TaxableTransaction(
                                    ticker, first_tx_in_sold_split, first_tx_out
                                )
                            )
                    else:
                        self.capital_gains_and_losses.append(
                    ...
                    first_tx_out_bought_split, first_tx_out_unbought_split = (
                        split_unequal_tx(
                            smaller_volume_tx=first_tx_in,
                            greater_volume_tx=first_tx_out,
                            greater_volume_tx_flow=TxFlow.OUT,
                        )
                    )
                    ...
                    running_total_tx_in_volume += first_tx_in.currency_out_volume
                    running_total_tx_out_volume += (
                        first_tx_out_bought_split.currency_in_volume
                    )
                    running_total_ticker_volume += (
                        first_tx_in.currency_out_volume
                        - first_tx_out_bought_split.currency_in_volume
                    ...
                    if self.tax_year:
                        if first_tx_out_bought_split.timestamp.year == self.tax_year:
                            self.capital_gains_and_losses.append(
                                TaxableTransaction(
                                    ticker, first_tx_in, first_tx_out_bought_split
                                )
                            )
                    else:
                        self.capital_gains_and_losses.append(
        ...
        return sorted(
            [
                taxable_tx
                for taxable_tx in self.capital_gains_and_losses
                if taxable_tx.capital_gain_or_loss != 0
            ],
    ...
    def get_taxable_income(self):
        ...
        return sorted(
            [
                taxable_tx
                for taxable_tx in taxable_income_txs
                if taxable_tx.cost_basis != 0
            ],
            key=lambda x: x.date_acquired,
        )

    def get_capital_gains_and_losses_df(self, exclude_columns: List[str] = None):
        return self._get_df_from_tx_list(
            self.get_capital_gains_and_losses(), exclude_columns
        )
    ...
    def get_taxable_income_df(self, exclude_columns: List[str] = None):
        ...
        taxable_income_df = self._get_df_from_tx_list(
            self.get_taxable_income(), exclude_columns
        )
        return taxable_income_df.loc[
            (
                to_datetime(taxable_income_df["date_acquired_str"], format="%m/%d/%Y")
                >= f"{self.tax_year}-01-01 00:00:00"
            )
            & (
                to_datetime(taxable_income_df["date_acquired_str"], format="%m/%d/%Y")
                < f"{self.tax_year + 1}-01-01 00:00:00"
            )
        ]
